...
class Block():
    def __init__(self, index=None, block_data=None, previous_hash=None, load=False, genesis=False):
        if(genesis == True):
            ...
            self.data = []
            if(load == False):
                # nothing can be NoneType
                # pid => person identifier
                # rid => reporting elf id
                # score => naughty/nice value
                # sign => naughty/nice sign (positive or negative)
                if all(p is not None for p in [index, block_data['documents'], block_data['pid'], block_data['rid'], block_data['score'], block_data['sign'], previous_hash]):
                    ...
                    if self.index == 0:
                        ...
                        self.nonce = random.randrange(0xFFFFFFFFFFFFFFFF)   # 64-bit => time-based seed?
    ...
    def __repr__(self):
        s = 'Chain Index: %i\n' % (self.index)
        s += '              Nonce: %s\n' % ('%016.016x' % (self.nonce)) # nonce in hex form e.g. Nonce: 000000000000006f
        s += '                PID: %s\n' % ('%016.016x' % (self.pid))   # person id in hex form
        s += '                RID: %s\n' % ('%016.016x' % (self.rid))   # elf id in hex form
        s += '     Document Count: %1.1i\n' % (self.doc_count)
        s += '              Score: %s\n' % ('%08.08x (%i)' % (self.score, self.score))
        # default to naughty LOL
        n_n = 'Naughty'
        if self.sign > 0:
            n_n = 'Nice'
        s += '               Sign: %1.1i (%s)\n' % (self.sign, n_n)
    ...
    def full_hash(self):
        hash_obj = MD5.new()
        # block_data_signed eturns a string of block_data + bytes of the block hash + block signature
